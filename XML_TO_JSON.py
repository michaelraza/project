...
"""
    if frequent_tag:
        dw_script += f"payload.{root.tag}.*{frequent_tag} map (item) -> {{\n"
        child_example = root.find(f".//{frequent_tag}")
        if child_example is not None:
            for child in child_example:
                if child.tag != "color_swatch":  # Ignore 'color_swatch' attributes
                    dw_script += f"  {child.tag}: $.{child.tag},\n"
                else:
                    dw_script += f"  // Ignored color_swatch elements\n"
            if child_example.attrib:
                for attr in child_example.attrib:
                    if attr != "image":  # Ignore 'image' attributes
